...
const int servoPins[8] = {8,10,6,4, 14, 13, 2, 1};

// Subtrim values for each servo (offset in degrees)
int8_t servoSubtrim[8] = {2, 0, 0, 6, 4, -2, 12, 4};
...
void setup() {
  ...
  for (int i = 0; i < 8; i++) {
      ...
      if (i == 4 || i == 6) { // R4 y L3 — servos diferentes
          servos[i].attach(servoPins[i], 500, 2500);

      } else {
          servos[i].attach(servoPins[i], 560, 3100);
      }
  }
  delay(10);
